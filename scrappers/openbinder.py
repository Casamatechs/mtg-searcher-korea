...
sys.path.append('..') # Add sibling dirs to path
...
class Openbinder:
    ...
    def server_call(self, name: str):
        return self.search_card_prices(name)
    ...
    def get_instock_urls(self, url: str, name: str) -> List[Card]:
        ...
        for card in in_stock:
            ...
            cards += self.search_card_price(self.BASE_URL + href, name, set_id)
        return cards
    ...
    def search_card_price(self, url: str, name: str, set_id: str) -> List[Card]:
        ...
        for store in normal.find_all('div',attrs={'class','card-body'}):
            ...
            set_cards.append(Card(name=name,lang=flag,cond=condition,store=store_name,price=int(re.sub(r'[^0-9]','',price)),stock=int(stock),foil=False))
        ...
        for store in foil.find_all('div',attrs={'class','card-body'}):
            ...
            price, stock = store.find('p').getText().split(' (')
            stock = stock.split(')')[0][5:-1]
            set_cards.append(Card(name=name,lang=flag,cond=condition,store=store_name,price=int(re.sub(r'[^0-9]','',price)),stock=int(stock),foil=True))
